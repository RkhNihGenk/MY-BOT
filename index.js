...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:RkhNihGenk!\n' 
            + 'ORG: Gua Jomblo!;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6282138746029:+62 821-3874-6029\n' 
            + 'END:VCARD'
prefix = 'z'
blocked = []   
limitawal = 25
...
const pacarNumber = ["6289999999999@s.whatsapp.net"]
/************************************/
...
const bgst = new WAConnection()
   bgst.on('qr', qr => {
   qrcode.generate(qr, { small: true })
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('bgst','red'),color('XSEC404','lime'))
})

bgst.on('credentials-updated', () => {
	const authInfo = bgst.base64EncodedAuthInfo()
   console.log(`credentials updated!`)
   fs.writeFileSync('./session.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./session.json') && bgst.loadAuthInfo('./session.json')
bgst.connect();


bgst.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await bgst.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await bgst.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `*Hallo* 👋 @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}*\nSemoga betah ya di sini 😅\nJangan lupa intro @${num.split('@')[0]} 😅`
				let buff = await getBuffer(ppimg)
				bgst.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await bgst.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
	...
	bgst.on('message-new', async (rkh) => {
		try {
			if (!rkh.message) return
			if (rkh.key && rkh.key.remoteJid == 'status@broadcast') return
			if (rkh.key.fromMe) return
			global.prefix
			global.blocked
			const content = JSON.stringify(rkh.message)
			const from = rkh.key.remoteJid
			const type = Object.keys(rkh.message)[0]
			const { text, extendedText, contact, location, liveLocation, image, video, sticker, document, audio, product } = MessageType
			const time = moment.tz('Asia/Jakarta').format('DD/MM HH:mm:ss')
			const timi = moment.tz('Asia/Jakarta').add(30, 'days').calendar();
			const timu = moment.tz('Asia/Jakarta').add(20, 'days').calendar();
			body = (type === 'conversation' && rkh.message.conversation.startsWith(prefix)) ? rkh.message.conversation : (type == 'imageMessage') && rkh.message.imageMessage.caption.startsWith(prefix) ? rkh.message.imageMessage.caption : (type == 'videoMessage') && rkh.message.videoMessage.caption.startsWith(prefix) ? rkh.message.videoMessage.caption : (type == 'extendedTextMessage') && rkh.message.extendedTextMessage.text.startsWith(prefix) ? rkh.message.extendedTextMessage.text : ''
			budy = (type === 'conversation') ? rkh.message.conversation : (type === 'extendedTextMessage') ? rkh.message.extendedTextMessage.text : ''
			var pes = (type === 'conversation' && rkh.message.conversation) ? rkh.message.conversation : (type == 'imageMessage') && rkh.message.imageMessage.caption ? rkh.message.imageMessage.caption : (type == 'videoMessage') && rkh.message.videoMessage.caption ? rkh.message.videoMessage.caption : (type == 'extendedTextMessage') && rkh.message.extendedTextMessage.text ? rkh.message.extendedTextMessage.text : ''
			...
			const q = args.join(' ')
			const botNumber = bgst.user.jid
			const sender = isGroup ? rkh.participant : rkh.key.remoteJid
			pushname = bgst.contacts[sender] != undefined ? bgst.contacts[sender].vname || bgst.contacts[sender].notify : undefined
			const groupMetadata = isGroup ? await bgst.groupMetadata(from) : ''
			...
			const reply = (teks) => {
				bgst.sendMessage(from, teks, text, {quoted:rkh})
			}
			const sendMess = (hehe, teks) => {
				bgst.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? bgst.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : bgst.sendMessage(from, teks.trim(), extendedText, {quoted: rkh, contextInfo: {"mentionedJid": memberr}})
			}
			const sendImage = (teks) => {
		    bgst.sendMessage(from, teks, image, {quoted:rkh})
		    }
		    const costum = (pesan, tipe, target, target2) => {
			bgst.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` }}})
			}
		    const sendPtt = (teks) => {
		    bgst.sendMessage(from, audio, mp3, {quoted:rkh})
		    }
	        /*****************END SCURITY FEATURE ********/
        ...
        if (levelRole <= 2) {
          ...
          const checkLimit = (sender) => {
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return bgst.sendMessage(from,`Limit request anda sudah habis\n\n_Note : limit bisa di dapatkan dengan cara ${prefix}buylimit dan dengan naik level_`, text,{ quoted: rkh})
                            bgst.sendMessage(from, ind.limitcount(limitCounts), text, { quoted : rkh})
                            found = true
                        }
                    }
                    if (found === false) {
           ...
           const isLimit = (sender) =>{ 
              ...
              if (limits >= limitawal ) {
              	  position = true
                    bgst.sendMessage(from, ind.limitend(pushname), text, {quoted: rkh})
                    return true
              } else {
              	_limit
                  position = true
		...
		bgst.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Link Group Terdeteksi maaf ${sender.split("@")[0]} anda akan di kick dari group 5detik lagi`)
		setTimeout( () => {
			bgst.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
			bgst.updatePresence(from, Presence.composing)
			reply("1detik")
		...
		setTimeout( () => {
			...
			switch(command) {
		case 'mutual': // Fix Bug By bgstXSec404				
				...
				break
		case 'next': // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				if (isGroup) return  reply( 'Command ini tidak bisa digunakan di dalam grup!')
				anug = getRegisteredRandomId(_registered).replace('@s.whatsapp.net','')
				await reply('Find for a partner...')
		...
		case 'transfer':
              ...
              case 'hii':
				...
				bgst.sendMessage(from, ainecs, MessageType.audio, { quoted: rkh, ptt: true })
				await limitAdd(sender)
				break
              case 'shizuka':
				 // Fix Bug By bgstXSec404				
				...
				bgst.sendMessage(from, ainezz, MessageType.audio, { quoted: rkh, ptt: true })
				await limitAdd(sender)
				break
              case 'soundbakahentai':
				...
				bgst.sendMessage(from, aineess, audio, {mimetype: 'audio/mp4', filename: `bakahentai.mp3`, quoted: rkh})
				await limitAdd(sender)
				break
              case 'sayonichan':
				 // Fix Bug By bgstXSec404				
				...
				bgst.sendMessage(from, aineesi, MessageType.audio, { quoted: rkh, ptt: true })
				await limitAdd(sender)
				break
              case 'soundplaydate':
				 // Fix Bug By bgstXSec404				
				...
				bgst.sendMessage(from, aineesiie, MessageType.audio, { quoted: rkh, ptt: true })
				await limitAdd(sender)
				break
              case 'hallo':
				 // Fix Bug By bgstXSec404				
				...
				bgst.sendMessage(from, aineesii, MessageType.audio, { quoted: rkh, ptt: true })
				await limitAdd(sender)
				break
                case 'admin':
				...
				bgst.sendMessage(from, teks.trim(), extendedText, {quoted: rkh, contextInfo: {"mentionedJid": adm}})
				break
                case 'premium':
				if (!isOwner) return reply(ind.ownerb())
				premm = body.slice(9)
		...
		case 'unpremium':
				...
				bgst.sendMessage(from, teks.trim(), extendedText, {quoted: rkh, contextInfo: {"mentionedJid": prem}})
				break
                case 'bann':
		...
		case 'unban':
				...
				bgst.sendMessage(from, teks.trim(), extendedText, {quoted: rkh, contextInfo: {"mentionedJid": ban}})
				break
		...
		case 'giftlimit': 
				...
				bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: `${teks}`})
				await limitAdd(sender)
				break
		case 'happymod':
				 // Fix Bug By bgstXSec404				
		...
		case 'shrtco':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					exec(`wget ${anu.url} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						bgst.sendMessage(from, buffer, sticker, {quoted: rkh})
						fs.unlinkSync(rano)
					})
					await limitAdd(sender)
					break
		...
		case 'randomhentai':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(ind.wait())
                                        ...
                                        bgst.sendMessage(from, buffer, image, {quoted: rkh})
					await limitAdd(sender)
                                        break
		case 'cium':
                                         // Fix Bug By bgstXSec404				
		...
		case 'nulis':
				...
				bgst.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: rkh})
				await limitAdd(sender)
				break
		case 'nulis2':
				 // Update By RzkyO & bgstXSec404				
		...
		/*case 'nulis2':
 			 ...
 			 //  reply('p\n' + util.format({fontPath, inputPath, outputPath, tgl, hari, textnulis}))
 				 ...
 				 .on('exit', () => {
  			  bgst.sendMessage(from, outputPath, image, {quoted: rkh, caption : ' nih sayang, jangan mager ya sayang'})
  			}))
  			  break*/
		case 'bplogo':
				 // Fix Bug By bgstXSec404				
				...
				buffer = await getBuffer(anu.result)
				bgst.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: rkh})
				await limitAdd(sender)
				break
		case 'hartatata':
				 // Fix Bug By bgstXSec404				
				...
				bgst.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: rkh})
				await limitAdd(sender)
				break
		case 'hartatata2':
				 // Update By RzkyO & bgstXSec404				
		...
		case 'shota':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				    try {
						...
						bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: 'Ingat! Cintai husbumu'})
					} catch (e) {
				...
				await limitAdd(sender)
				break
				case 'tolol': // Update By RzkyO & bgstXSec404
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				...
				await limitAdd(sender)
				break
				case 'emoji2img': // Update By RzkyO & bgstXSec404
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'randomkpop':
				...
				buffer = await getBuffer(anu.result)
				bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: 'Nih kpopnya kak...'})
				await limitAdd(sender)
				break
		case 'aesthetic':
				...
				bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: 'Nih aestheticnya kak...'})
				await limitAdd(sender)
				break
                case 'joox':
				 // Fix Bug By bgstXSec404				
				...
				bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: infomp3})
				bgst.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${data.result.title}.mp3`, quoted: rkh})
				await limitAdd(sender)
				break
		case 'play':   
				 // Fix Bug By bgstXSec404				
				...
				bgst.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: rkh})
				await limitAdd(sender)
				break
		/*case 'play2':   
				 // Fix Bug By bgstXSec404				
				...
				buffer = await getBuffer(anu.result.thumbnail)
				bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: iinfomp3})
				laguu = await getBuffer(anu.result.url)
				bgst.sendMessage(from, laguu , audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: rkh})
				await limitAdd(sender)
				break*/
		case 'limit':
				 // Fix Bug By bgstXSec404				
		...
		case 'vinta':
				...
				bgst.sendMessage(from, vintage, image, {caption: 'nih anjim ${vin}', quoted: rkh})
				await limitAdd(sender)
				break
		case 'avengers':
				 // Fix Bug By bgstXSec404				
				...
				bgst.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: rkh})
				await limitAdd(sender)
				break 
		case 'summer':
				 // Fix Bug By bgstXSec404				
		...
		case 'firework':
				...
				bgst.sendMessage(from, arugazzz, image, {caption: 'Nih kak', quoted: rkh})
				await limitAdd(sender)
				break
		case 'lava':
				 // Fix Bug By bgstXSec404				
                ...
                case '1cak':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
						...
						bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: 'Nih Kak udah Jadi'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(ind.wrongf())
					}
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break	
                case 'faktaunik': // Fixed By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'quotes':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'covidindo': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break		
		case 'covidglobal': // Update By bgstXSec404 & RzkyO   
				 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				bgst.updatePresence(from, Presence.composing) 
				...
				await limitAdd(sender)
					break
		case 'ceknamaff': // Update By bgstXSec404 & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					...
					await limitAdd(sender)
					break
		case 'ceknamaml': // Update By bgstXSec404 & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
					...
					await limitAdd(sender)
					break
		case 'namaninja': // Update By bgstXSec404 & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(11)
		...
		case 'infogempa':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					hasil = `*Potensi*\n${anu.potensi}\n*Lokasi*\n${anu.lokasi}\n*Magnitude*\n${anu.magnitude}\n*Koordinat*\n${anu.koordinat}\n*Kedalaman*\n${anu.kedalaman}\n*Waktu*\n${anu.waktu}\n*Map*\n${anu.map}`
					bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: hasil})
					await limitAdd(sender)
					break
		case 'infocuaca':
					tels = body.slice(11)
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, hasil, text, {quoted: rkh})
					await limitAdd(sender)
					break
		case 'github': // Update By RzkyO & bgstXSec404
					tels = body.slice(8) 
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, buffer7, image, {quoted: rkh, caption: capt})
					await limitAdd(sender)
					break
                case 'slap':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, pod, image, { quoted: rkh, caption: '*Toxic*\n\n'+ ple })
					await limitAdd(sender)
					break
		case 'tampar':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
			    case 'liputan6': // Update By RzkyO & bgstXSec404
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					bgst.updatePresence(from, Presence.composing) 
					...
					await limitAdd(sender)
					break
				case 'foxnews': // Update By RzkyO & bgstXSec404
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					bgst.updatePresence(from, Presence.composing) 
					...
					await limitAdd(sender)
					break
				case 'nickff': // Update By RzkyO & bgstXSec404
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					bgst.updatePresence(from, Presence.composing) 
					...
					await limitAdd(sender)
					break
				case 'newsinfo': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break 
				case 'film': // Update By RzkyO & bgstXSec404	
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					bgst.updatePresence(from, Presence.composing) 
					...
					await limitAdd(sender)
					break
				case 'film2': // Update By RzkyO & bgstXSec404	
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					bgst.updatePresence(from, Presence.composing) 
		...
		case 'setppbot':
					...
					enmedia = JSON.parse(JSON.stringify(rkh).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await bgst.downloadAndSaveMediaMessage(enmedia)
					await bgst.updateProfilePicture(botNumber, media)
		...
		case 'brainly':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, teks, text, {quoted: rkh, detectLinks: false})
					console.log(res)
					})
					await limitAdd(sender)
					break 
		case 'bcgc':
					...
					tagss = rkh.participant
					if (isMedia && !rkh.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(rkh).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rkh
						buffer = await bgst.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							bgst.sendMessage(_.jid, buffer, image, {caption: `*「 BC GROUP 」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
		...
		case 'pinterest':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					nirkh =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nirkh)
					bgst.sendMessage(from, pok, image, { quoted: rkh, caption: `*PINTEREST*`})
					await limitAdd(sender)
					break 
           ...
           case 'imagemountain':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.shizukaa.xyz/api/wpmountain?apikey=tsmerkh633`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					bgst.sendMessage(from, buffer, image, {quoted: rkh })
					await limitAdd(sender) 
					break
		   ...
		   case 'mimpi':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
			        ...
			        bgst.sendMessage(from, mimpi, text, {quoted: rkh })
			        await limitAdd(sender) 
			    	break
		     case 'fakta':
			         // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					...
					bgst.sendMessage(from, fakta, text, {quoted: rkh })
					await limitAdd(sender) 
					break 
				case 'katabijak':
			         // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					...
					bgst.sendMessage(from, katabijak, text, {quoted: rkh })
					await limitAdd(sender) 
					break
			case 'katailham':
			         // Fix Bug By bgstXSec404				
				...
				kata = anu.result
				bgst.sendMessage(from, kata, text, {quoted: rkh })
				await limitAdd(sender)
				break
				case 'caklontong':
			         // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					bgst.sendMessage(from, '*тЮ╕ Jawaban :* '+anu.jawaban+ '\n\nтАв Penjelasan: *'+ anu.keterangan+'*', text, {quoted: rkh }) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					bgst.sendMessage(from, '_10 Detik lagiтАж_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					bgst.sendMessage(from, '_20 Detik lagi_тАж', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					bgst.sendMessage(from, '_30 Detik lagi_тАж', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
					bgst.sendMessage(from, caklontong, text, {quoted: rkh }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
		         case 'babi':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                 const gshizuka = await bgst.getGroupMembersId(groupId)
                 let gmik = gshizuka[Math.floor(Math.random() * gshizuka.length)]
                 const mmkk = `YANG PALING BABI DISINI ADALAH @${gmik.replace(/@c.us/g, '')}`
                 bgst.sendTextWithMentions(dari, mmkk, id)
                 break
				case 'tebakgambar':
			         // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					bgst.sendMessage(from, '*тЮ╕ Jawaban :* '+anu.jawaban, text, {quoted: rkh }) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					bgst.sendMessage(from, '_10 Detik lagiтАж_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					bgst.sendMessage(from, bufferkkk, image, { caption: '_Jelaskan Apa Maksud Gambar Ini_', quoted: rkh }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break  
		case 'spamcall':
			...
			bgst.sendMessage(from, `${anu.result.logs}`, text, {quoted: rkh })
			await limitAdd(sender) 
			break  
				case 'wiki':
			         // Fix Bug By bgstXSec404				
                ...
                if (anu.error) return reply(anu.error)
                wikii = `${anu.result}`
                bgst.sendMessage(from, wikii, text, {quoted: rkh })
                await limitAdd(sender) 
                break
               case 'asupan':
			         // Fix Bug By bgstXSec404				
                ...
                bgst.sendMessage(from, asup, video, {mimetype: 'video/mp4', filename: `asupan_bangsa.mp4`, quoted: rkh, caption: 'Asupannya Tuan:v'})
                await limitAdd(sender) 
                break
               case 'ytmp4':
			         // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					...
					reply(mess.wait)
					buffer = await getBuffer(anu.result)
					bgst.sendMessage(from, buff, image, {quoted: shizuka, caption: ytt})
					bgst.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: shizuka, caption: 'Nih Gan'})
					await limitAdd(sender) 
					break 
			case 'ytmp3':
			         // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					...
					reply(mess.wait)
					buffer = await getBuffer(anu.result)
					bgst.sendMessage(from, buff, image, {quoted: shizuka, caption: teks})
					bgst.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: rkh })
					await limitAdd(sender) 
					break
			 case 'nsfwloli':
				    try {
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
						...
						bgst.sendMessage(from, buffer, image, {quoted: shizuka, caption: 'Jangan jadiin bahan buat comli om'})
					    } catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					    }
		...
		case 'hilih': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'holoh': // Update By bgstXSec404 & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'halah': // Update By bgstXSec404 & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'huluh': // Update By bgstXSec404 & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'heleh': // Update By bgstXSec404 & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'hurufterbalik': // Update By bgstXSec404 & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					...
					await limitAdd(sender)
					break
		case 'lirik': // Update By bgstXSec404 & RzkyO
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					teks = body.slice(6)
		...
		case 'alay':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'alay2': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'fml':	// Fixed By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'jadwaltv':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'katadilan': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'katabucin': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'katabucin2': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'cerpen': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'quotes': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'bacotanhacker': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'quotedoraemon': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'igstalk':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'infofilm': // Update By bgstXSec404 & RzkyO
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break
		case 'heroml':  // Update By RzkyO & bgstXSec404
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(`[❕] Loading`)
				...
				bgst.sendMessage(from, buffer7, image, {quoted: rkh, caption: teks})
				await limitAdd(sender)
				break
					case 'igvideo': // Update By bgstXSec404 & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					...
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
                    bgst.sendMessage(from, buffer, video, )
				    break
				case 'igpost': // Update By bgstXSec404 & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Urlnya mana tong?')
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					...
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
                    bgst.sendMessage(from, buffer, image, )
				    break
				case 'igtv': // Update By bgstXSec404 & RzkyO				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Urlnya mana tong?')
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
                    ...
                    bgst.sendMessage(from, buffer, video, )
				    break
				case 'tiktoknowm': // Update By RzkyO & bgstXSec404
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Urlnya mana tong?')
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.lv)
                    ...
                    bgst.sendMessage(from, buffer, video, )
				    break
				 case 'ccgenerator': // Update By RzkyO & bgstXSec404
					if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                   reply(`_[❕] Loading_`)
                ...
                case 'kickall':
					...
					bgst.groupRemove(from, members_id)
					break*/
		case 'setreply':
					if (!isOwner) return reply(ind.ownerb())
					bgst.updatePresence(from, Presence.composing) 
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					bgst.sendMessage(from, teks.trim(), extendedText, {quoted: rkh})
					break
		//daftar 
		case 'daftar':
                			...
                			if (umurUser < 12) return reply(`your age is too young minimum 12 years`)
                					try {
								ppimg = await bgst.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
								ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
							}
                					veri = sender
                    			...
                    			await bgst.sendMessage(from, ppimg, image, {quoted: rkh, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
                    			addATM(sender)
		...
		case 'bisakah':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: rkh })
					await limitAdd(sender)
					break
		case 'kapankah':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: rkh })
					await limitAdd(sender)
					break
		case 'apakah':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: rkh })
					await limitAdd(sender)
					break
		case 'bagaimanakah':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+bagaimanakah+'*\n\nJawaban : '+ mana, text, { quoted: rkh })
					await limitAdd(sender)
					break
		case 'rate':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+rate+'*\n\nJawaban : '+ te+'%', text, { quoted: rkh })
					await limitAdd(sender)
					break
                case 'sangecek':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+sange+'*\n\nJawaban : '+ nge+'%', text, { quoted: rkh })
					await limitAdd(sender)
					break
                case 'gaycek':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+gayy+'*\n\nJawaban : '+ yag+'%', text, { quoted: rkh })
					await limitAdd(sender)
					break
                case 'lesbicek':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+lesbii+'*\n\nJawaban : '+ bi+'%', text, { quoted: rkh })
					await limitAdd(sender)
					break
                case 'gantengcek':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+ganteng+'*\n\nJawaban : '+ teng+'%', text, { quoted: rkh })
					await limitAdd(sender)
					break
		case 'cantikcek':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+cantik+'*\n\nJawaban : '+ tik+'%', text, { quoted: rkh })
					await limitAdd(sender)
					break
		case 'watak':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+watak+'*\n\nJawaban : '+ tak, text, { quoted: rkh })
					await limitAdd(sender)
				        break
		case 'hobby':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: rkh })
					await limitAdd(sender)
					break
		case 'speed5':
		case 'ping3':
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					await bgst.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
					break
		case 'help': 
		case 'menu':
					 // Fix Bug By bgstXSec404				
		...
		case 'donate':
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					bgst.sendMessage(from, donasi(), text)
					break
		case 'menupicture':
		case 'menupict':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, buffer, image, { quoted: rkh, caption: randompict(prefix)})
					break
		case 'menuowner':
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, buffer, image, { quoted: rkh, caption: developer(prefix)})
					break
		case 'bahasa':
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					bgst.sendMessage(from, bahasa(), text)
					await limitAdd(sender)
					break
		case 'kodenegara':
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					bgst.sendMessage(from, negara(), text)
					await limitAdd(sender)
					break
		case 'del':
		case 'd':
		case 'delete':
				...
				if (!isPremium) return reply('Maaf kamu bukan user premium!')
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					bgst.deleteMessage(from, { id: rkh.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					await limitAdd(sender)
					break
		case 'level':
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					...
					bgst.sendMessage(from, resul, text, { quoted: rkh})
					.catch(async (err) => {
					console.error(err)
					await reply(`Error!\n${err}`)
					})
		...
		case 'say':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        ...
                                        saying = teks
                                        bgst.sendMessage(from, saying, text)
					await limitAdd(sender)
                                        break
		case 'info':
					me = bgst.user
					...
					bgst.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
		case 'snk':
					me = bgst.user
					uptime = process.uptime()
		...
		case 'lpr':
					...
					if (pesan.length > 300) return bgst.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: rkh})
					var nomor = rkh.participant
					teks1 = `*[LAPORAN]*\nNomor : @${nomor.split("@s.whatsapp.net")[0]}\nPesan : ${bug}`
					var options = {
					text: teks1,
					contextInfo: {mentionedJid: [nomor]},
					}
					bgst.sendMessage('6281281872699@s.whatsapp.net', options, text, {quoted: rkh})
					reply('Masalah telah di laporkan ke owner Shizuka BOT, Laporan palsu atau main² tidak akan ditanggapi.')
					break
		case 'request':
					const cfrr = body.slice(8)
					if (cfrr.length > 300) return bgst.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: rkh})
					var nomor = rkh.participant
					...
					bgst.sendMessage('6281281872699@s.whatsapp.net', options, text, {quoted: rkh})
					bgst.sendMessage('6281315113882@s.whatsapp.net', options, text, {quoted: rkh})
					bgst.sendMessage('6282148618789@s.whatsapp.net', options, text, {quoted: rkh})
		...
		case 'blocklist': 
					...
					bgst.sendMessage(from, teks.trim(), extendedText, {quoted: rkh, contextInfo: {"mentionedJid": blocked}})
					break
                case 'hidetag':
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					var group = await bgst.groupMetadata(from)
					...
					bgst.sendMessage(from, options, text)
					await limitAdd(sender)
					break
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					buffer = await getBuffer(anu.result)
					bgst.sendMessage(from, buffer, image, {caption: 'Nih anjim', quoted: rkh})
					await limitAdd(sender)
					break
                case 'truth':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: rkh })
					await limitAdd(sender)
					break
		case 'dare':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, tod, image, { quoted: rkh, caption: '*Dare*\n\n'+ der })
					await limitAdd(sender)
					break
		case 'quoteskehidupan':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, crot, image, { quoted: rkh, caption: '*Quotes Kehidupan*\n\n'+ quo })
					await limitAdd(sender)
					break
		case 'quotesislami':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, islam, image, { quoted: rkh, caption: '*Quotes Islami*\n\n'+ isl })
					await limitAdd(sender)
					break	
		case 'quotesnasehat':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, nase, image, { quoted: rkh, caption: '*Quotes Nasehat*\n\n'+ nsh })
					await limitAdd(sender)
					break	
		case 'quotescinta':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, cta, image, { quoted: rkh, caption: '*Quotes Cinta*\n\n'+ cin })
					await limitAdd(sender)
					break
		case 'quotesmotivasi':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, vass, image, { quoted: rkh, caption: '*Motivasi*\n\n'+ cin })
					await limitAdd(sender)
					break
                case 'fotocewek':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
	                                const cangti = ['https://i.ibb.co/1T1DCz7/cewek-thailand-20200325-007-non-fotografer-kly.jpg','https://i.ibb.co/FsJ6jjs/kucing2.jpg','https://i.ibb.co/vvkdS7n/kucing3.jpg','https://i.ibb.co/1QHWxts/kucing4.jpg','https://i.ibb.co/JQmRz4n/kucing5.jpg','https://i.ibb.co/tBwrFkG/kucing6.jpg','https://i.ibb.co/dp0YhYm/kucing7.jpg','https://i.ibb.co/R03smZT/kucing8.jpg','https://i.ibb.co/17tw0dp/kucing9.jpg','https://i.ibb.co/7XdGGqc/kucing10.jpg','https://i.ibb.co/XL9PZxg/kucing11.jpg','https://i.ibb.co/gyjvXWN/kucing12.jpg','https://i.ibb.co/R4gg4wH/kucing13.jpg','https://i.ibb.co/PmLYtFm/kucing14.jpg','https://i.ibb.co/XbSDh47/kucing15.jpg','https://i.ibb.co/kSXNJzt/kucing16.jpg']
                                        ...
                                        bgst.sendMessage(from, cangtip, image, {quote: rkh})
					await limitAdd(sender)
                                        break
                /*case 'kucing':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
	                                const kucings = ['https://i.ibb.co/vvmp82w/kucing1.jpg']
                                        ...
                                        bgst.sendMessage(from, kucigz, image, {quote: rkh})
					await limitAdd(sender)
                                        break*/
		case 'ssweb':
					 // Fix Bug By bgstXSec404				
		...
		case 'waifu':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, hasil, image, {quoted: rkh})
					await limitAdd(sender)
					break
		case 'loli2':
					 // Fix Bug By bgstXSec404				
		...
		case 'pokemon':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					var nirkh =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nirkh)
					bgst.sendMessage(from, pok, image, { quoted: rkh })
					await limitAdd(sender)
					break
                case 'anjing':
					 // Fix Bug By bgstXSec404				
		...
		case 'map':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					buffer = await getBuffer(anu.gambar)
					bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: `${body.slice(5)}`})
					await limitAdd(sender)
					break
                case 'kbbi':
					 // Fix Bug By bgstXSec404				
		...
		case 'quran':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, quran, text, {quoted: rkh})
					await limitAdd(sender)
					break
		case 'jadwalsholat':
					 // Fix Bug By bgstXSec404				
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					bgst.sendMessage(from, sholat, text, {quoted: rkh})
					await limitAdd(sender)
					break
                case 'neonime':
				         // Fix Bug By bgstXSec404				
		...
		case 'ocr': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if ((isMedia && !rkh.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(rkh).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rkh
						const media = await bgst.downloadAndSaveMediaMessage(encmedia)
						reply(ind.wait())
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					if ((isMedia && !rkh.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.toFormat('webp')
							.save(ran)
					} else if ((isMedia && rkh.message.videoMessage.seconds < 11 || isQuotedVideo && rkh.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) && args.length == 0) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(rkh).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rkh
						const media = await bgst.downloadAndSaveMediaMessage(encmedia)
						ran = getRandom('.webp')
						reply(ind.wait())
						await ffmpeg(`./${media}`)
		...
		case 'tts':
				...
				if (!isPremium) return reply('Maaf kamu bukan user premium!')
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (args.length < 1) return bgst.sendMessage(from, 'Diperlukan kode bahasa kak!!', text, {quoted: rkh})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return bgst.sendMessage(from, 'Mana teks yang mau di jadiin suara? suara setan kah?', text, {quoted: rkh})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							buffer = fs.readFileSync(rano)
							if (err) return reply(ind.stikga())
							bgst.sendMessage(from, buffer, audio, {quoted: rkh, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					await limitAdd(sender)
		...
		case 'tiktokstalk':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				try {
						if (args.length < 1) return bgst.sendMessage(from, '*Username mana kak?', text, {quoted: rkh})
				...
				linkgc = await bgst.groupInviteCode (from)
				yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
				bgst.sendMessage(from, yeh, text, {quoted: rkh})
				await limitAdd(sender)
				break
		case 'sayang':
					bgst.sendMessage(from, 'Apa sayangku? 🥰',MessageType.text, { quoted: rkh} )
					break
		case 'gelud':
					bgst.sendMessage(from, 'Kuylah, dapat duit aku..',MessageType.text, { quoted: rkh} )
					break
		case 'bot':
					bgst.sendMessage(from, 'Iya kak? \nJangan lupa daftar iya kak ^_^"\nJika sudah silahkan berarti sudah terdaftar kak ^_^"',MessageType.text, { quoted: rkh} )
					break
		case 'hargaprem':
		case 'hargapremium':
					bgst.sendMessage(from, 'Level 100 = Free 3 day (premium)\n10k = 7day (premium)\n25k = bulan (premium)',MessageType.text, { quoted: rkh} )
					break
		case 'buypremium':
					bgst.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/6281281872699',MessageType.text, { quoted: rkh} )
					break
		case 'buypremunlimited':
					if (!isPremium) return reply('Maaf kamu bukan user premium!')
					bgst.sendMessage(from, 'Selamat kamu sudah berlangganan selamanya 🎉',MessageType.text, { quoted: rkh} )
					break
		case 'buyprem30day':
					if (!isPremium) return reply('Maaf kamu bukan user premium!')
					bgst.sendMessage(from, 'Selamat kamu sudah berlangganan 30 hari',MessageType.text, { quoted: rkh} )
					break
		case 'buyprem7day':
					if (!isPremium) return reply('Maaf kamu bukan user premium!')
					bgst.sendMessage(from, 'Selamat kamu sudah berlangganan 7 hari',MessageType.text, { quoted: rkh} )
					break
		case 'test':
					bgst.sendMessage(from, 'Active',MessageType.text, { quoted: rkh} )
					break
		case 'ping':
					bgst.sendMessage(from, 'Active',MessageType.text, { quoted: rkh} )
					break
		case 'assalamualaikum':
					bgst.sendMessage(from, 'Waalaikumusalam',MessageType.text, { quoted: rkh} )
					break
		case 'kontol':
					bgst.sendMessage(from, 'Gunakan bahasa yang benar\nAnda akan kami banned!\nHubungi kami : wa.me/6281281872699',MessageType.text, { quoted: rkh} )
					break
		case 'merkh':
					bgst.sendMessage(from, 'Gunakan bahasa yang benar\nAnda akan kami banned!\nHubungi kami : wa.me/6281281872699',MessageType.text, { quoted: rkh} )
					break
		...
		case 'wame':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
  					...
  					bgst.sendMessage(from, options, text, { quoted: rkh } )
  					break
  					if (data.error) return reply(data.error)
  					reply(data.result)
					await limitAdd(sender)
		...
		case 'clearall':
					if (!isOwner) return reply(ind.ownerb())
					anu = await bgst.chats.all()
					bgst.setMaxListeners(25)
					for (let _ of anu) {
						bgst.deleteChat(_.jid)
					}
					reply(ind.clears())
					break
		case 'blockk':
				 ...
				 bgst.chatRead (from)
					if (!isGroup) return reply(ind.groupo())
				if (!isAdmin) return reply('*Only Admin bot*')
					bgst.blockUser (`${body.slice(8)}@c.us`, "add")
					bgst.sendMessage(from, `*Perintah Diterima, Memblokir* ${body.slice(7)}@c.us`, text)
					break
		case 'unblockk':
					if (!isGroup) return reply(ind.groupo())
				if (!isAdmin) return reply('*Only Admin bot*')
				    bgst.blockUser (`${body.slice(10)}@c.us`, "remove")
					bgst.sendMessage(from, `*Perintah Diterima, Membuka Blockir* ${body.slice(9)}@c.us`, text)
					break
		case 'block':
				 bgst.updatePresence(from, Presence.composing) 
				 bgst.chatRead (from)
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
					bgst.blockUser (`${body.slice(7)}@c.us`, "add")
					bgst.sendMessage(from, `*Perintah Diterima, Memblokir* ${body.slice(7)}@c.us`, text)
					break
		case 'unblock':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				    bgst.blockUser (`${body.slice(9)}@c.us`, "remove")
					bgst.sendMessage(from, `*Perintah Diterima, Membuka Blockir* ${body.slice(9)}@c.us`, text)
					break
		case 'leave':
					 // Fix Bug By bgstXSec404				
                 if (!isRegistered) return reply( ind.noregis())
					...
					if (!isOwner) return reply(ind.ownerb())
					setTimeout( () => {
					bgst.groupLeave (from) 
					}, 2000)
					setTimeout( () => {
					bgst.updatePresence(from, Presence.composing) 
					bgst.sendMessage(from, 'Sampai jumpa 👋', text) // ur cods
					}, 0)
					break
		case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
					...
					if (isMedia && !rkh.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(rkh).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rkh
						buff = await bgst.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							bgst.sendMessage(_.jid, buff, image, {caption: `*「 PESAN BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('*Suksess broadcast* ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 BGST BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
		...
		case 'setpp': 
					...
					if (!isGroupAdmins) return reply(ind.admin())
                   			if (!isBotGroupAdmins) return reply(ind.badmin())
					media = await bgst.downloadAndSaveMediaMessage(rkh)
					await bgst.updateProfilePicture (from, media)
		...
		case 'group':
					...
					if (args[0] === 'buka') {
						...
						bgst.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`*BERHASIL MENUTUP GROUP*`)
						bgst.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
					break      
            case 'admin':
            case 'owner':
            case 'creator':
            case 'developer':
                  bgst.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: rkh})
                  bgst.sendMessage(from, 'Tuh nomer owner ku >_<, jangan spam atau ku block kamu',MessageType.text, { quoted: rkh} )
					break    
           /*case 'setname':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					bgst.groupUpdateSubject(from, `${body.slice(9)}`)
					bgst.sendMessage(from, 'Succes, Ganti Nama Grup', text, {quoted: rkh})
					break*/
                /*case 'setdesc':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					bgst.groupUpdateDescription(from, `${body.slice(9)}`)
					bgst.sendMessage(from, 'Succes, Ganti Deskripsi Grup', text, {quoted: rkh})
					break*/
           /*case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (rkh.message.extendedTextMessage === undefined || rkh.message.extendedTextMessage === null) return reply('*Tag target yang ingin di turunkan admin group!*')
					mentioned = rkh.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						bgst.groupDemoteAdmin(from, mentioned)
					} else {
			...
			/*case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (rkh.message.extendedTextMessage === undefined || rkh.message.extendedTextMessage === null) return reply('*Tag target yang ingin di jadikan admin group!*')
					mentioned = rkh.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(teks, mentioned, true)
						bgst.groupMakeAdmin(from, mentioned)
					} else {
		...
		case 'kick':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (rkh.message.extendedTextMessage === undefined || rkh.message.extendedTextMessage === null) return reply('𝗧𝗮𝗴 𝘁𝗮𝗿𝗴𝗲𝘁 ??𝗮𝗻𝗴 𝗶𝗻𝗴𝗶𝗻 𝗱𝗶 𝘁𝗲𝗻𝗱𝗮𝗻𝗴!')
					mentioned = rkh.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(teks, mentioned, true)
						bgst.groupRemove(from, mentioned)
					} else {
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Reply/Tag sticker!*')
					reply(ind.wait())
					encmedia = JSON.parse(JSON.stringify(rkh).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await bgst.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						bgst.sendMessage(from, buffer, image, {quoted: rkh, caption: '*DAH JADI SETAN* '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
		...
		case 'welcome':
                 ...
                 case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						bgst.sendMessage(from,`Perhatian kepada seluruh member anti link group aktif apabila anda mengirim link group anda akan di kick dari group`, text)
					} else if (Number(args[0]) === 0) {
		...
		case 'clone':
					...
					if (rkh.message.extendedTextMessage === undefined || rkh.message.extendedTextMessage === null) return reply('❬ SUCCSESS ❭')
					mentioned = rkh.message.extendedTextMessage.contextInfo.mentionedJid[0]
					let { jid, id, notify } = groupMembers.find(x => x.jid === mentioned)
					try {
						pp = await bgst.getProfilePicture(id)
						buffer = await getBuffer(pp)
						bgst.updateProfilePicture(botNumber, buffer)
					...
					Axios.get('https://berita.news/')
					.then(({ data }) => {
     					const $ = cheerio.load(data)
     					let title = []
     					let url = []
     					let img = []
 					$('div.td_module_10 > div > a').get().map((rest) => {
         				url.push($(rest).attr('href'))
         				})
         				$('div.td_module_10 > div > a').get().map((rest) => {
        					title.push($(rest).attr('title'))
         				})
         				$('div.td_module_10 > div > a > img').get().map((rest) => {
					...
					bgst.sendMessage(from, teks, MessageType.text)
					})
					break
					case 'citacita':
					halo = await getBuffer(`https://pencarikode.xyz/api/cita-cita?apikey=Fummy`)
					bgst.sendMessage(from, halo, audio, {quoted: tod, mimetype: 'audio/mp4', ptt:true, duration : 999999999999})
					break
					case 'swfitnah':
				
					bataz = args.join(' ').replace(/@|\d/gi, '').split('|')
					tagg = tod.message.extendedTextMessage.contextInfo.mentionedJid[0]
					bgst.sendMessage(from, `${bataz[1]}`, text, {quoted: { key: { fromMe: false, participant: `${tagg}`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: {"extendedTextMessage":{"text":`${bataz[0]}`,"textArgb":4294967295,"backgroundArgb":4280563264,"font":"SANS_SERIF","previewType":"NONE"}}}})
					break
					case 'zipfitnah':
				
					batazk = args.join(' ').replace(/@|\d/gi, '').split('|')
					todsz = tod.message.extendedTextMessage.contextInfo.mentionedJid[0]
					bgst.sendMessage(from, `${batazk[1]}`, text, {quoted:  { key: { fromMe: false, participant: `${todsz}`, ...(from ? { remoteJid: from } : {}) }, message: {"documentMessage":{"url":"https://mmg.whatsapp.net/d/f/AuLPdGoiCKWGgnQrLd-SQsjk22vMrh4LpkbnH0EcC7rP.enc","mimetype":"application/zip","title":`${batazk[0]}`,"fileSha256":"qxX+rr2T73ABMy+7Ogu4mAKCuVdY2UGLxp18RrcskrY=","fileLength":"14777","pageCount":0,"mediaKey":"noTHCT7BdJKeflrwxhvIU/vZ6RD7SKnI+ebultd0Ks4=","fileName":"bgst-bot.zip","fileEncSha256":"PsfZZXIrer20kExBrLIP8xHEJ9C4KYbudDele2St3vY=","directPath":"/v/t62.7119-24/31313965_416397753034191_4323442961335371179_n.enc?oh=750fb2dd296cec450f58b874a8c74530&oe=602EE546","mediaKeyTimestamp":"1610851064"}}}})
											break
											case 'fitnah':
                 case 'f':
				...
				mentioned = tod.message.extendedTextMessage.contextInfo.mentionedJid
					...
					var bot = gh.split("|")[2];
					bgst.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
					break
					case 'fakedeface':
                                  
                                    if (args.length < 1) return reply('Contoh https://youtube.be|text|text')
                                        gh = body.slice(12)
                                        gh1 = gh.split("|")[0];
                                        gh2 = gh.split("|")[1];
                                        gh3 = gh.split("|")[2];
                                        ...
                                        bgst.sendMessage(from, `${gh1}`, text, {thumbnail: fs.readFileSync(`./${run}`),title : `${gh2}`,desc: `${gh3}`})
				...
				case 'wait':
					...
					if ((isMedia && !rkh.message.videoMessage || isQuotedImage) && args.length == 0) {
						reply(ind.wait())
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(rkh).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rkh
						media = await bgst.downloadMediaMessage(encmedia)
						await wait(media).then(res => {
							bgst.sendMessage(from, res.video, video, {quoted: rkh, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
						})
					} else {
